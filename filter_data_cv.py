# This script does the filtering as to construct 
# good CSMs on the *entire* data set, not to
# test performance on a held out test set.
# 
#   DMD, 200114-14-34
...
ofile = open('filtering_results-ICWSM-2.tsv', 'w')
...
for index, user_num in enumerate(range(3487, len(users))):
    numpy.random.seed(user_num)

    print 'On user {} of {}'.format(user_num, len(users))
    ...
    fname = '/Volumes/ddarmon-external/Reference/R/Research/Data/tweetpredict/timeseries_2011/byday-600s-{}'.format(suffix)

    try:
        open('/Volumes/ddarmon-external/Reference/R/Research/Data/tweetpredict/timeseries_2011/byday-600s-{}.dat'.format(suffix))
        ...
        ofile.write('{}\t{}\t{:.4}\t{:.4}\t{}\t{}\t{}\t{}\n'.format(users[user_num], user_num, baseline_rates[index], cm_rates[index], n_states[index], Cmus[index], hmus[index], Lopts[index]))
